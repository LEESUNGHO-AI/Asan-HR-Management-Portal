    ...
    for b in blocks:
        ...
        if bt == "callout":
            ...
            if "기준월" in txt:
                for p in txt.split("|"):
                    p = p.strip()
                    if "기준월" in p: data["base_month"] = p.split(":")[-1].strip().split()[0]
                    if "등록인원" in p:
                        m = re.search(r"(\d+)", p); data["total"] = int(m.group(1)) if m else 0
                    if "활성인원" in p:
                        m = re.search(r"(\d+)", p); data["active"] = int(m.group(1)) if m else 0
                    if "업데이트" in p: data["update_date"] = p.split(":")[-1].strip().split()[0]
...
<div class="org-card-body"><table class="data-table"><thead><tr><th>성명</th><th>직급</th><th>역할</th><th>투입률</th><th>투입기간</th><th>상태</th></tr></thead><tbody>{arows}{erows}</tbody></table></div></div>\n'''
...
<p>📅 {today} | ✍️ Notion 자동 동기화 | 📧 <a href="mailto:smartcity-pmo@cheileng.com">PMO 문의</a></p>
